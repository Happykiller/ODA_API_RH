...
//Liens de test
// API_RH/phpsql/getDetailsJour.php?milis=123450&ctrl=ok&date=2014-05-19&code_user=FRO
...
if($arrayValeur["error"] == null){
    ...
    //--------------------------------------------------------------------------
    $strSql = "Select a.`id`, a.`code_user`, a.`statut`, a.`am_ap`
        ...
        ON 1=1
        WHERE 1=1
        AND a.`date` = '".$arrayValeur["date"]."'
        AND a.`actif` = 1
        ORDER BY a.`code_user`, a.`am_ap`
    ";
    ...
    $object_retour->data["detailsJour"] = $resultats;
